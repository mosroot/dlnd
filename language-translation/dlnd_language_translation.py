...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    """
    source_text_id = [[source_vocab_to_int[word] for word in sentence.split()] for sentence in source_text.split('\n')]
    target_text_id = [[target_vocab_to_int[word] for word in sentence.split()] + [target_vocab_to_int['<EOS>']] for sentence in target_text.split('\n')]
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob):
    ...
    """
    # TODO: Implement Function
    en_cell = tf.contrib.rnn.MultiRNNCell([tf.contrib.rnn.BasicLSTMCell(rnn_size)] * num_layers)
    
    en_cell = tf.contrib.rnn.DropoutWrapper(en_cell, output_keep_prob = keep_prob)      
    
    rnn, state = tf.nn.dynamic_rnn(en_cell, rnn_inputs, dtype = tf.float32)
    return state
...
def decoding_layer(dec_embed_input, dec_embeddings, encoder_state, vocab_size, sequence_length, rnn_size,
    ...
    dec_cell = tf.contrib.rnn.MultiRNNCell([rnn] * num_layers)
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size, sequence_length, source_vocab_size, target_vocab_size,
    ...
    targets = process_decoding_input(target_data, target_vocab_to_int, batch_size)
    
    dec_embeddings = tf.Variable(tf.truncated_normal([target_vocab_size, dec_embedding_size], stddev=0.01))
    targets_embed = tf.nn.embedding_lookup(dec_embeddings, targets)
